...
async def crawl(config: Config):
    """Scrapes theneedledrop.com for album review URLs and their corresponding HTML content stored in json format."""
    visited_pages = set()
    results = []
...
if __name__ == "__main__":
    current_crawler_config = Config(
        ...
        output_file_name=os.path.join(os.path.dirname(os.path.abspath(__file__)), "output_test.json")
    )
