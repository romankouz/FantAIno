...
class NeighborhoodComponentsModel(FantAInoFitter):
    """Neighborhood Components model for FantAIno."""

    def __init__(
        self,
        mode = "regression",
        n_neighbors: int = 5,
        nca_components: int = 5,
        ...
        match self.mode:
            case "regression":
                self.base_model = Pipeline([
                    ("nca", NeighborhoodComponentsAnalysis(n_components=nca_components)),
                    ("knn", KNeighborsRegressor(n_neighbors=n_neighbors, n_jobs=self.n_jobs))
                ])
                self.model_name = "NCA Regressor"
            case "classification":
                self.base_model = Pipeline([
                    ("nca", NeighborhoodComponentsAnalysis(n_components=nca_components)),
                    ("knn", KNeighborsClassifier(n_neighbors=n_neighbors, n_jobs=self.n_jobs))
                ])
                self.model_name = "NCA Classifier"
            case _:
